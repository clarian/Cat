...
updater () {
	...
	if hash git 2>/dev/null; then
		...
		git remote add origin https://github.com/clarian/cat.git >/dev/null 2>&1
...
run_bot() {
	...
	if hash python3 2>/dev/null; then # TODO abstracify all this which mirrors above an also look up boolean operators in sh
		if hash pip3 2>/dev/null; then
			...
			echo "Upgrading pip"
			if python3.6 -m pip install --upgrade pip; then
				echo "Upgrading requirements"
				if python3.6 -m pip install -r requirements.txt >/dev/null; then
					echo "Starting bot..."
					python3.6 bot.py
					ret=$?
					if [ $ret == "15" ]; then
